 ...
 * Runs inside Surge http-request script and serves a local web UI at https://subviz.local/
...
var SubViz = (function () {
  ...
  function respond(status, body, headers) {
    ...
    if (typeof $done === 'function') {
      $done({ response: { status: status || 200, headers: base, body: body || '' } });
    }
  }
  ...
  function parseQuery(url) {
    var q = '';
    var idx = url.indexOf('?');
    if (idx >= 0) q = url.slice(idx + 1).split('#')[0];
    var out = {};
    if (!q) return out;
  ...
  function maybeDecodeWhole(raw) {
    ...
    if (looksLikeBase64(s)) {
      ...
      if (/^(vmess|vless|trojan|ss|ssr|hysteria2|hy2|tuic|snell):\/\//im.test(decoded) || /proxies\s*:/i.test(decoded) || decoded.split('\n').length > 2) {
        return decoded;
      }
    }
    return s;
  ...
  function parseClashProxies(text) {
    ...
    for (var i = 0; i < lines.length; i++) {
      if (/^\s*proxies\s*:\s*$/i.test(lines[i])) { start = i + 1; break; }
    }
    if (start < 0) return [];
    var nodes = [], current = null;
    function pushCurrent() {
      if (!current) return;
      if (current.name || current.server || current.type) nodes.push(clashToNode(current));
      current = null;
    }
      ...
      if (mStart) {
        ...
        if (rest) {
          if (rest.charAt(0) === '{') current = parseInlineMap(rest);
            ...
            var kv0 = parseYamlBlockValue(rest);
            if (kv0) current[kv0.key] = kv0.value;
          }
        }
        continue;
      }
  ...
  var FLAG_TO_CODE = {
    '🇭🇰': 'HK', '🇹🇼': 'TW', '🇯🇵': 'JP', '🇸🇬': 'SG', '🇺🇸': 'US', '🇰🇷': 'KR', '🇬🇧': 'GB',
    '🇩🇪': 'DE', '🇫🇷': 'FR', '🇨🇦': 'CA', '🇦🇺': 'AU', '🇷🇺': 'RU', '🇳🇱': 'NL', '🇮🇹': 'IT',
    '🇪🇸': 'ES', '🇹🇷': 'TR', '🇮🇳': 'IN', '🇹🇭': 'TH', '🇻🇳': 'VN', '🇲🇾': 'MY', '🇵🇭': 'PH',
    '🇮🇩': 'ID', '🇲🇴': 'MO', '🇨🇳': 'CN'
  };
  ...
  function inferCountry(name, server) {
    ...
    for (var flag in FLAG_TO_CODE) {
      if (text.indexOf(flag) >= 0) {
        var c = FLAG_TO_CODE[flag];
        return { code: c, name: CODE_TO_NAME[c] || c };
      }
    }
    for (var i = 0; i < COUNTRY_RULES.length; i++) {
      if (COUNTRY_RULES[i][2].test(text)) return { code: COUNTRY_RULES[i][0], name: COUNTRY_RULES[i][1] };
    }
    var m = String(server || '').toLowerCase().match(/\.([a-z]{2})(?:\.|$)/);
    if (m) {
      var tld = m[1].toUpperCase();
      if (CODE_TO_NAME[tld]) return { code: tld, name: CODE_TO_NAME[tld] };
    }
  ...
  function parseHostPort(s) {
    ...
    if (s.charAt(0) === '[') {
      var end = s.indexOf(']');
      var host = end >= 0 ? s.slice(1, end) : s;
      var rest = end >= 0 ? s.slice(end + 1) : '';
      return { server: host, port: rest.charAt(0) === ':' ? rest.slice(1) : '' };
    }
    var parts = s.split(':');
    if (parts.length > 1) {
      var port = parts.pop();
      return { server: parts.join(':'), port: port };
    }
  ...
  function parseUrlParams(q) {
    ...
    q.split('&').forEach(function (part) {
      var idx = part.indexOf('=');
      var k = safeDecodeURIComponent(idx >= 0 ? part.slice(0, idx) : part);
  ...
  function parseVmess(url) {
    var payload = String(url).replace(/^vmess:\/\//i, '').split('#')[0].trim();
  ...
  function parseSSR(url) {
    var payload = String(url).replace(/^ssr:\/\//i, '').trim();
    var decoded = base64Decode(payload);
    var parts = decoded.split('/?');
    var main = parts[0] || '';
    var params = parseUrlParams(parts[1] || '');
    var seg = main.split(':');
    var name = params.remarks ? base64Decode(params.remarks) : '';
  ...
  function parseSS(url) {
    ...
    var main = body;
    if (main.indexOf('@') < 0) {
      var decoded = base64Decode(main);
      if (decoded.indexOf('@') >= 0) main = decoded;
    } else {
      var left = main.split('@')[0];
      if (left.indexOf(':') < 0) {
        var decodedLeft = base64Decode(left);
        main = decodedLeft + '@' + main.split('@').slice(1).join('@');
      }
    }
    var hp = parseHostPort(main.split('@').pop());
    var extra = { query: parseUrlParams(query) };
  ...
  function parseGenericUri(url) {
    var raw = String(url);
    var schemeMatch = raw.match(/^([a-z0-9+.-]+):\/\//i);
    var protocol = schemeMatch ? schemeMatch[1].toLowerCase() : 'unknown';
    var body = raw.replace(/^[a-z0-9+.-]+:\/\//i, '');
    var name = '';
    var hidx = body.indexOf('#');
    ...
    var qidx = body.indexOf('?');
    if (qidx >= 0) { query = body.slice(qidx + 1); body = body.slice(0, qidx); }
    var authority = body.split('/')[0];
    var hostPart = authority.indexOf('@') >= 0 ? authority.split('@').pop() : authority;
    var hp = parseHostPort(hostPart);
    var params = parseUrlParams(query);
    var network = params.type || params.network || params.transport || '';
    var tls = params.security || params.tls || '';
    return makeNode({ protocol: protocol === 'hy2' ? 'hysteria2' : protocol, name: name || params.name || hp.server, server: hp.server, port: hp.port, network: network, tls: tls, sourceFormat: 'uri', raw: url, extra: params });
  }
  ...
  function clashToNode(obj) {
    obj = obj || {};
    return makeNode({
      protocol: obj.type || obj.protocol || 'unknown',
      name: obj.name || obj.server || '未命名节点',
      server: obj.server || obj.add || '',
      port: obj.port || '',
      network: obj.network || obj.type_transport || obj.transport || '',
      tls: obj.tls || obj.security || '',
  ...
  function extractUris(text) {
    var s = String(text || '');
    var re = /\b(vmess|vless|trojan|ssr|ss|hysteria2|hy2|hysteria|tuic|snell|socks5|socks):\/\/[^\s"'<>\\]+/ig;
    var out = [], m;
    while ((m = re.exec(s))) {
  ...
  function parseSubscription(raw) {
    var normalized = maybeDecodeWhole(raw);
    var nodes = [];
    ...
    // Deduplicate accidental double extraction by exact raw.
    var seenRaw = {}, clean = [];
    nodes.forEach(function (n) {
      var key = n.raw || (n.protocol + n.name + n.server + n.port);
      if (seenRaw[key]) return;
      seenRaw[key] = true; clean.push(n);
    });
  ...
  function duplicateGroups(nodes) {
    ...
    return Object.keys(map).filter(function (k) { return map[k].length > 1; }).sort(function (a, b) { return map[b].length - map[a].length; }).map(function (k) {
      return { fingerprint: k, count: map[k].length, nodes: map[k] };
    });
  }

  function analyze(raw, meta) {
    meta = meta || {};
    var parsed = parseSubscription(raw);
    var nodes = parsed.nodes;
    var dupGroups = duplicateGroups(nodes);
    ...
    return {
      ...
      normalizedSize: parsed.normalizedText.length,
      summary: {
        total: nodes.length,
        unique: uniqueCount,
        duplicates: nodes.length - uniqueCount,
        protocols: countBy(nodes, 'protocol').length,
        countries: countBy(nodes, 'country').length
      },
      stats: {
        byProtocol: countBy(nodes, 'protocol'),
        byCountry: countBy(nodes, 'country'),
        byCountryCode: countBy(nodes, 'countryCode'),
        bySourceFormat: countBy(nodes, 'sourceFormat')
      },
  ...
  function fetchUrl(url, callback) {
    if (typeof $httpClient === 'undefined') {
      callback(new Error('当前环境没有 $httpClient，仅 Surge 脚本运行时可拉取远程订阅。'), null, null);
      return;
    }
    $httpClient.get({
      url: url,
      headers: {
        'User-Agent': 'SubViz-Surge/' + VERSION,
        'Accept': '*/*'
      }
  ...
  function handleAnalyzeByUrl(url) {
    if (!/^https?:\/\//i.test(url || '')) {
      respondJSON({ ok: false, error: '请输入 http/https 订阅链接。' }, 400); return;
    }
  ...
  function handleAnalyzeText(body) {
    try {
      ...
      if (/^\s*\{/.test(raw)) {
        ...
        raw = obj.raw || obj.text || '';
        source = obj.sourceName || source;
      }
  ...
  function handleSample() {
    ...
    var sample = [
      vmess,
      'trojan://password@jp.example.com:443?security=tls#🇯🇵JP-日本 01',
      'vless://uuid@sg.example.com:443?encryption=none&security=tls&type=ws#🇸🇬SG-新加坡 01',
      'ss://' + b64('aes-128-gcm:pass@us.example.com:8388') + '#🇺🇸US-美国 01',
      'trojan://password@jp.example.com:443?security=tls#🇯🇵JP-日本 01 副本'
    ].join('\n');
  ...
  function htmlPage() {
    return '<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover">' +
      ...
      '<section class="panel"><div class="panel-title">去重统计</div><div id="duplicates" class="dups hint">暂无重复节点。</div></section>' +
      '<footer>本工具仅在 Surge 本地脚本中解析订阅，不提供订阅转换、不保存订阅内容。导出操作发生在浏览器端。</footer>' +
      '</main><script>' + clientJS() + '</script></body></html>';
  }

  function css() {
    return ':root{--bg:#0f172a;--card:#111c33;--card2:#0b1220;--line:#24344f;--text:#e5edf8;--muted:#92a2b8;--accent:#7dd3fc;--accent2:#c084fc;--good:#86efac;--warn:#fbbf24}*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at top left,#1f3b68 0,#0f172a 38%,#08111f 100%);color:var(--text);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,"PingFang SC","Microsoft YaHei",sans-serif}.app{max-width:1180px;margin:0 auto;padding:28px 18px 48px}.hero{display:flex;align-items:flex-start;justify-content:space-between;gap:20px;margin:18px 0 24px}.eyebrow{letter-spacing:.15em;text-transform:uppercase;color:var(--accent);font-size:12px}.hero h1{font-size:40px;line-height:1.08;margin:8px 0;background:linear-gradient(90deg,#fff,#bfe9ff,#e9d5ff);-webkit-background-clip:text;color:transparent}.sub{color:var(--muted);max-width:760px;line-height:1.7}.badge{padding:9px 14px;border:1px solid var(--line);border-radius:999px;background:rgba(255,255,255,.06);color:#c7d2fe}.panel{background:linear-gradient(180deg,rgba(17,28,51,.92),rgba(11,18,32,.92));border:1px solid var(--line);box-shadow:0 20px 60px rgba(0,0,0,.24);border-radius:22px;padding:18px;margin-bottom:16px}.input-panel label,.panel-title{font-weight:700;font-size:16px}.row{display:flex;gap:10px;margin-top:10px}input,textarea,select{width:100%;background:#07101f;border:1px solid #263a58;color:var(--text);border-radius:14px;padding:12px 13px;outline:none}textarea{min-height:150px;margin:12px 0;resize:vertical}input:focus,textarea:focus,select:focus{border-color:var(--accent);box-shadow:0 0 0 3px rgba(125,211,252,.12)}button{border:0;border-radius:14px;padding:12px 16px;background:linear-gradient(135deg,#38bdf8,#a78bfa);color:#06111f;font-weight:800;cursor:pointer;white-space:nowrap}.ghost,.secondary{background:rgba(255,255,255,.07);color:var(--text);border:1px solid var(--line)}button:hover{filter:brightness(1.07)}details{margin-top:12px;color:var(--muted)}summary{cursor:pointer}.status{margin-top:12px;color:var(--muted);font-size:13px}.status.ok{color:var(--good)}.status.err{color:#fca5a5}code{background:#06101f;border:1px solid var(--line);padding:2px 5px;border-radius:6px}.grid{display:grid;gap:16px}.cards{grid-template-columns:repeat(5,minmax(0,1fr))}.card{padding:16px;border-radius:20px;background:rgba(255,255,255,.055);border:1px solid var(--line)}.card .num{font-size:30px;font-weight:900}.card .lab{color:var(--muted);font-size:13px;margin-top:4px}.two{grid-template-columns:1fr 1fr}.chart{display:flex;flex-direction:column;gap:10px;margin-top:14px}.bar{display:grid;grid-template-columns:95px 1fr 46px;gap:10px;align-items:center;font-size:13px}.bar .track{height:12px;border-radius:999px;background:#07101f;overflow:hidden;border:1px solid #263a58}.bar .fill{height:100%;border-radius:999px;background:linear-gradient(90deg,#38bdf8,#c084fc)}.toolbar{display:flex;justify-content:space-between;gap:14px;align-items:flex-start}.filters{display:flex;gap:8px;flex-wrap:wrap;justify-content:flex-end}.filters input{width:220px}.filters select{width:130px}.check{display:flex;align-items:center;gap:6px;color:var(--muted);font-size:13px}.check input{width:auto}.hint{color:var(--muted);font-size:13px;margin:6px 0 0}.table-wrap{overflow:auto;margin-top:12px;border:1px solid var(--line);border-radius:16px}table{width:100%;border-collapse:collapse;min-width:980px;background:rgba(6,16,31,.42)}th,td{padding:11px 10px;text-align:left;border-bottom:1px solid rgba(36,52,79,.7);font-size:13px;vertical-align:top}th{position:sticky;top:0;background:#101b31;color:#bfdbfe;z-index:1}td.mono{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;color:#c7d2fe}.pill{display:inline-flex;align-items:center;padding:4px 8px;border-radius:999px;background:rgba(125,211,252,.12);color:#bfdbfe;border:1px solid rgba(125,211,252,.25);font-weight:700;font-size:12px}.dups{display:flex;flex-direction:column;gap:8px;margin-top:12px}.dup{padding:10px;border:1px solid var(--line);background:rgba(255,255,255,.04);border-radius:14px}.dup b{color:var(--warn)}footer{color:var(--muted);text-align:center;font-size:12px;margin-top:20px}@media(max-width:860px){.hero{flex-direction:column}.hero h1{font-size:32px}.cards,.two{grid-template-columns:1fr}.row,.toolbar{flex-direction:column}.filters{justify-content:flex-start}.filters input,.filters select{width:100%}button{width:100%}}';
  }

  function clientJS() {
    return "var data=null,filtered=[];var $=function(id){return document.getElementById(id)};function setStatus(s,c){var el=$('status');el.textContent=s;el.className='status '+(c||'')}function esc(s){return String(s==null?'':s).replace(/[&<>\"]/g,function(m){return {'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;'}[m]})}function pct(n,d){return d?Math.round(n*100/d):0}async function getJSON(url,opt){var r=await fetch(url,opt||{});var t=await r.text();try{var j=JSON.parse(t)}catch(e){throw new Error(t.slice(0,200)||'接口返回异常')}if(!r.ok||j.ok===false)throw new Error(j.error||('HTTP '+r.status));return j}function saveLast(){try{localStorage.setItem('subviz:lastUrl',$('url').value)}catch(e){}}function loadLast(){try{$('url').value=localStorage.getItem('subviz:lastUrl')||''}catch(e){}}async function analyzeUrl(){var url=$('url').value.trim();if(!url)return setStatus('请先输入订阅 URL。','err');saveLast();setStatus('正在拉取并解析订阅……');try{render(await getJSON('/api/analyze?url='+encodeURIComponent(url)));setStatus('分析完成。','ok')}catch(e){setStatus(e.message,'err')}}async function analyzeText(){var raw=$('raw').value;if(!raw.trim())return setStatus('请先粘贴订阅内容。','err');setStatus('正在解析粘贴内容……');try{render(await getJSON('/api/analyze-text',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({raw:raw})}));setStatus('分析完成。','ok')}catch(e){setStatus(e.message,'err')}}async function sample(){setStatus('正在载入演示数据……');try{render(await getJSON('/api/sample'));setStatus('演示数据已载入。','ok')}catch(e){setStatus(e.message,'err')}}function render(d){data=d;renderSummary();renderCharts();fillFilters();applyFilters();renderDups()}function renderSummary(){var s=data.summary;var items=[['总节点',s.total],['唯一节点',s.unique],['重复节点',s.duplicates],['协议数',s.protocols],['地区数',s.countries]];$('summary').innerHTML=items.map(function(x){return '<div class=card><div class=num>'+x[1]+'</div><div class=lab>'+x[0]+'</div></div>'}).join('')}function bars(arr,total){if(!arr||!arr.length)return '<p class=hint>暂无数据</p>';return arr.slice(0,12).map(function(x){return '<div class=bar><div>'+esc(x.key)+'</div><div class=track><div class=fill style=\"width:'+pct(x.count,total)+'%\"></div></div><div>'+x.count+'</div></div>'}).join('')}function renderCharts(){$('protocolChart').innerHTML=bars(data.stats.byProtocol,data.summary.total);$('countryChart').innerHTML=bars(data.stats.byCountry,data.summary.total)}function fillFilters(){var ps=data.stats.byProtocol.map(function(x){return x.key});var cs=data.stats.byCountry.map(function(x){return x.key});$('protocolFilter').innerHTML='<option value=\"\">全部协议</option>'+ps.map(function(x){return '<option>'+esc(x)+'</option>'}).join('');$('countryFilter').innerHTML='<option value=\"\">全部地区</option>'+cs.map(function(x){return '<option>'+esc(x)+'</option>'}).join('')}function uniqueNodes(nodes){var seen={};return nodes.filter(function(n){if(seen[n.fingerprint])return false;seen[n.fingerprint]=1;return true})}function applyFilters(){if(!data)return;var kw=$('kw').value.trim().toLowerCase(),p=$('protocolFilter').value,c=$('countryFilter').value;var nodes=$('uniqueOnly').checked?uniqueNodes(data.nodes):data.nodes;filtered=nodes.filter(function(n){var hay=[n.name,n.server,n.protocol,n.country,n.countryCode].join(' ').toLowerCase();return(!kw||hay.indexOf(kw)>=0)&&(!p||n.protocol===p)&&(!c||n.country===c)});renderTable()}function renderTable(){var tb=$('tbody');$('tableHint').textContent='当前显示 '+filtered.length+' / '+(data?data.nodes.length:0)+' 个节点';tb.innerHTML=filtered.map(function(n,i){return '<tr><td>'+(i+1)+'</td><td>'+esc(n.name)+'</td><td><span class=pill>'+esc(n.protocol)+'</span></td><td>'+esc(n.country)+' <span class=hint>'+esc(n.countryCode)+'</span></td><td class=mono>'+esc(n.server)+'</td><td class=mono>'+esc(n.port)+'</td><td>'+esc([n.network,n.tls].filter(Boolean).join(' / '))+'</td><td class=mono>'+esc(n.fingerprint)+'</td></tr>'}).join('')||'<tr><td colspan=8 class=hint>没有符合筛选条件的节点。</td></tr>'}function renderDups(){if(!data)return;var d=data.duplicates;if(!d.length){$('duplicates').textContent='暂无重复节点。';return}$('duplicates').innerHTML=d.slice(0,30).map(function(g){return '<div class=dup><b>'+g.count+' 个重复</b><div class=mono>'+esc(g.fingerprint)+'</div><div>'+g.nodes.map(function(n){return esc(n.name)}).join('、')+'</div></div>'}).join('')}function download(name,type,content){var blob=new Blob([content],{type:type});var a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=name;document.body.appendChild(a);a.click();setTimeout(function(){URL.revokeObjectURL(a.href);a.remove()},1000)}function csvCell(v){v=String(v==null?'':v);return /[\",\n]/.test(v)?'\"'+v.replace(/\"/g,'\"\"')+'\"':v}function exportCsv(){if(!data)return setStatus('暂无可导出数据。','err');var rows=[['name','protocol','country','countryCode','server','port','network','tls','fingerprint']].concat(filtered.map(function(n){return [n.name,n.protocol,n.country,n.countryCode,n.server,n.port,n.network,n.tls,n.fingerprint]}));download('subviz-nodes.csv','text/csv;charset=utf-8',rows.map(function(r){return r.map(csvCell).join(',')}).join('\n'))}function exportJson(){if(!data)return setStatus('暂无可导出数据。','err');download('subviz-analysis.json','application/json;charset=utf-8',JSON.stringify({summary:data.summary,stats:data.stats,duplicates:data.duplicates,nodes:filtered},null,2))}document.addEventListener('DOMContentLoaded',function(){loadLast();$('analyze').onclick=analyzeUrl;$('sample').onclick=sample;$('analyzeText').onclick=analyzeText;$('kw').oninput=applyFilters;$('protocolFilter').onchange=applyFilters;$('countryFilter').onchange=applyFilters;$('uniqueOnly').onchange=applyFilters;$('exportCsv').onclick=exportCsv;$('exportJson').onclick=exportJson});";
  }
  ...
  function run() {
    ...
    if (path === '/api/health') { respondJSON({ ok: true, name: 'SubViz Surge', version: VERSION, time: nowIso() }); return; }
