var SUBVIZ_SURGE_0_1_22 = true;
var SubViz = (function () {
  'use strict';
  var VERSION = '0.1.22';
  var MARKER = 'SUBVIZ_SURGE_0_1_22';
  ...
  function nodePolicyDescriptor(node) {
    ...
    var params = [], desc = '';
    var sni = firstOf(e, ['sni','servername','serverName','server-name']);
    ...
    if (type === 'ss') {
      ...
      if (surgeBool(firstOf(e, ['tls'])) || clean(e.security) === 'tls') addParam(params, 'tls', 'true');
      if (sni) addParam(params, 'sni', sni);
      if (skip) addBoolParam(params, 'skip-cert-verify', skip);
      if (/^ws$/i.test(network)) {
        addParam(params, 'ws', 'true');
      ...
      var pbk = firstOf(e, ['public-key','publicKey']);
      var sid = firstOf(e, ['short-id','shortId']);
      if (pbk) addParam(params, 'public-key', pbk);
      if (sid) addParam(params, 'short-id', sid);
      if (udp) addBoolParam(params, 'udp-relay', udp);
    } else if (type === 'tuic') {
      ...
      return { ok:false, error:'unsupported protocol for landing test: ' + type };
    }
    if (params.length) desc += ', ' + params.join(', ');
    return { ok:true, descriptor: desc, protocol:type, server:server, port:port };
  }
  ...
  function landingLookup() {
    ...
    var node = null;
    try { node = JSON.parse(body || '{}'); } catch (e) { return respondJSON({ ok:false, error:'invalid node json: '+String(e) }, 400); }
    ...
    function tryAPI(apiIndex, retryNo) {
      if (apiIndex >= apis.length) return respondJSON({ ok:false, error:'landing lookup failed', errors: errors.slice(-8), attempts: attemptNo, descriptorProtocol: built.protocol, server: built.server, port: built.port }, 502);
      var api = apis[apiIndex];
      var provider = internal ? 'internal' : landingApiName(api);
      var started = Date.now();
      var opt = { url: api, timeout: timeout, insecure: true, headers: { 'User-Agent': 'SubViz/' + VERSION }, 'policy-descriptor': built.descriptor };
      attemptNo++;
      $httpClient.get(opt, function (err, resp, data) {
        var status = Number((resp && (resp.status || resp.statusCode)) || 0);
        if (err || (status && status >= 400)) {
          errors.push({ api: api, provider: provider, error: String(err || ('HTTP ' + status)), retry: retryNo });
          if (retryNo < retries) return setTimeout(function () { tryAPI(apiIndex, retryNo + 1); }, retryDelay || 0);
          return tryAPI(apiIndex + 1, 0);
        }
        try {
          ...
          if (internal) {
            ...
            if (!r) errors.push({ api: api, provider: provider, error: 'internal geoip failed or $utils unsupported', ip: ip });
          } else {
            try { rawObj = JSON.parse(data || '{}'); } catch (e1) { rawObj = { ip: getBodyIP(data) }; }
            r = normalizeGeoResult(rawObj, provider, built.server);
            if (!r) errors.push({ api: api, provider: provider, error: (rawObj && (rawObj.message || rawObj.error)) || 'parse failed', raw: String(data || '').slice(0, 160) });
          }
          if (!r) {
            if (retryNo < retries) return setTimeout(function () { tryAPI(apiIndex, retryNo + 1); }, retryDelay || 0);
            return tryAPI(apiIndex + 1, 0);
          ...
          errors.push({ api: api, provider: provider, error: String(e), raw: String(data || '').slice(0, 160) });
          if (retryNo < retries) return setTimeout(function () { tryAPI(apiIndex, retryNo + 1); }, retryDelay || 0);
          return tryAPI(apiIndex + 1, 0);
        }
      });
